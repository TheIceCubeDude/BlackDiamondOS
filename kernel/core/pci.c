...
void pci_set_interrupt_pin(struct pci_device device, u8 pin) {_pci_write(device, 16, pin << 8); return;}
void pci_set_interrupt_line(struct pci_device device, u8 line) {_pci_write(device, 16, line); return;}
...
u32 pci_get_bar_address(struct pci_device device, u8 bar) {
	u32 raw_bar = pci_get_bar(device, bar) & 0xF;
